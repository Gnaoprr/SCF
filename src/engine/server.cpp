...
#endif
		serverslice(true, 5);
        if(server::quit) { server::close(); break; }
	}
...
void initserver(bool listen, bool dedicated)
{
    ...
    #ifndef _WIN32
        int i = system("rm -rf scripts/temp; mkdir scripts/temp");
        if(i != 0) conoutf("Emptying temp folder failed with error %i", i);
    #endif
    execfile("server-init.cfg", false);
    execfile("scripts/autorun/init.cfg", false);
