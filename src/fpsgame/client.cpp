...
namespace game
{
    ...
    void parsemessages(int cn, fpsent *d, ucharbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            {
                int model = getint(p);
                if(d && !scfAnswerModelAuth(model, d))
                {
                    d->playermodel = model;
			...
			case N_SCFSERVERAUTH: {
				int version = getint(p); // May be required in future development.
				conoutf("\f0[SCF-Client]\f7: Server is running on \f1SCF-Server \f7version \f0%i\f7.", version);
    ...
    void receivefile(packetbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            case N_SCFSCRIPT: // SCF servers may deliver scritps to client (as seen in scflib or scflibx addons)
            {
                if(!scfServer) return;
                string text;
                getstring(text, p);
            ...
            case N_SCFMAP: // Map delievered by scf servers if client's using modded map
            {
                ...
                map->write(b.buf, b.maxlen);
                delete map;
                if(load_world(mname[0] ? mname : NULL, mname[0] ? mname : NULL))
                    entities::spawnitems(true);
                break;
            }
        }
