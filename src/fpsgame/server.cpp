...
namespace server
{
    ...
    VAR(competitive, 0, 0, 1);

    void autoteam()
    {
        static const char * const teamnames[2] = {competitive ? "CT" : "good", competitive ? "T" : "evil"};
    ...
    vector<on_setmaster> onsetmaster;
    ICOMMAND(flush_onsetmaster, "", (), {
        loopvrev(onsetmaster) {
            onsetmaster.remove(i);
        }
    })
    ICOMMAND(onsetmaster, "s", (const char *command), {
        if(!command) return;
        on_setmaster *cur = &onsetmaster.add();
        if(!cur) return;
        copystring(cur->command, command);
    })
    ...
    bool setmaster(clientinfo *ci, bool val, const char *pass = "", const char *authname = NULL, const char *authdesc = NULL, int authpriv = PRIV_MASTER, bool force = false, bool trial = false)
    {
        ...
        defformatstring(cmd)("scf_setmaster_cn = %i; scf_setmaster_priv = %i; scf_setmaster_authname = \"%s\"; scf_setmaster_authdesc = \"%s\"", ci->clientnum, ci->privilege, authname ? authname : "", authdesc ? authdesc : "");
        execute(cmd);

        loopv(onsetmaster) {
            execute(onsetmaster[i].command);
        }
        return true;
    }
    ...
    vector<on_kick> onkick;
    ICOMMAND(flush_onkick, "", (), {
        loopvrev(onkick) {
            onkick.remove(i);
        }
    })
    ICOMMAND(onkick, "s", (const char *command), {
        if(!command) return;
        on_kick *cur = &onkick.add();
        if(!cur) return;
        copystring(cur->command, command);
    })
    VAR(kickhours, 0, 4, 64);
    ...
    bool trykick(clientinfo *ci, int victim, const char *reason = NULL, const char *authname = NULL, const char *authdesc = NULL, int authpriv = PRIV_NONE, bool trial = false)
    {
        ...
        if((priv || ci->local) && ci->clientnum!=victim)
        {
            ...
            if(vinfo && vinfo->connected && (priv >= vinfo->privilege || ci->local) && vinfo->privilege < PRIV_OWNER && !vinfo->local)
            {
                ...
                defformatstring(cmd)("scf_kick_actor = %i; scf_kick_victim = %i; scf_kick_time = %i; scf_kick_authname = \"%s\"; scf_kick_authdesc = \"%s\"", ci->clientnum, victim, 4*60*60, authname ? authname : "", authdesc ? authdesc : "");
                execute(cmd);

                loopv(onkick) {
                    execute(onkick[i].command);
                }
                uint ip = getclientip(victim);
                addban(ip, kickhours*60*60000);
    ...
    vector<on_changemap> onchangemap;
    ICOMMAND(flush_onchangemap, "", (), {
        loopvrev(onchangemap) {
            onchangemap.remove(i);
        }
    })
    ICOMMAND(onchangemap, "s", (const char *command), {
        if(!command) return;
    ...
    vector<on_frag> onfrag;
    ICOMMAND(flush_onfrag, "", (), {
        loopvrev(onfrag) {
            onfrag.remove(i);
        }
    })
    ICOMMAND(onfrag, "s", (const char *command), {
        if(!command) return;
        on_frag *cur = &onfrag.add();
        if(!cur) return;
        copystring(cur->command, command);
    })
    ...
    void dodamage(clientinfo *target, clientinfo *actor, int damage, int gun, const vec &hitpush = vec(0, 0, 0))
    {
        ...
        if(ts.health<=0)
        {
            defformatstring(cmd)("scf_frag_actor = %i; scf_frag_target = %i; scf_frag_gun = %i", actor->clientnum, target->clientnum, gun);
            execute(cmd);

            loopv(onfrag) {
                execute(onfrag[i].command);
            }
    ...
    void suicide(clientinfo *ci)
    {
        ...
        if(t) t->frags += fragvalue;
        defformatstring(cmd)("scf_frag_actor = %i; scf_frag_target = %i; scf_frag_gun = %i", ci->clientnum, ci->clientnum, ci->state.gunselect);
            execute(cmd);
    ...
    vector<servcmd> servcmds;

#define INTCONST(name, constant) ICOMMAND(name, "", (), { \
    intret(constant); \
})

    ICOMMAND(addcommand, "ssii", (const char *name, const char *file, int *privilege, int *override), {
        if(!name || !file || !privilege) return;
        bool ov = *override && *override == 1;
        loopv(servcmds) {
            if(!strcmp(servcmds[i].name, name)) {
                if(!ov) return;
                else {
                    servcmds.remove(i);
                }
        ...
        servcmd *cur = &servcmds.add();
        if(!cur) return;
        copystring(cur->name, name);
        copystring(cur->file, file);
        cur->privilege = *privilege;
    });

    void runcommand(int cn, const char *args, servcmd *command) {
        string cmd;
        formatstring(cmd)("scf_command_cn = %i; scf_command_args = \"%s\"; exec scripts/%s", cn, args, command->file);
        execute(cmd);
    }

    void parsecommand(clientinfo *ci, char *text) {
        if(!ci || ci->state.aitype != AI_NONE) return;
        char *array[2];
        explodestr(text, array, ' ', 2);
        if(!array[0]) return;
        const char *cmd  = (const char *)array[0],
                   *args = array[1] ? (const char *)array[1] : "";
        servcmd *command = 0;
        loopv(servcmds) {
            if(!strcmp(servcmds[i].name, cmd)) {
                command = &servcmds[i];
                break;
            }
        }
        if(!command) {
            string msg;
            formatstring(msg)("Unknown command: %s", cmd);
            sendf(ci->clientnum, 1, "ris", N_SERVMSG, msg);
            return;
        }
        if(command->privilege > ci->privilege) {
    ...
    // Main command links.
    ICOMMAND(servmsg, "s", (const char *msg), {
        sendf(-1, 1, "ris", N_SERVMSG, msg);
    });
    ICOMMAND(clientmsg, "is", (int *cn, const char *msg), {
        clientinfo *ci = getinfo(*cn);
        if(!ci || ci->state.aitype != AI_NONE) return;
        sendf(ci->clientnum, 1, "ris", N_SERVMSG, msg);
    });
    ICOMMAND(isvalidcn, "i", (int *cn), {
        if(!cn) {
            intret(0);
            return;
        }
    ...
    ICOMMAND(isbot, "i", (int *cn), {
        ...
        clientinfo *ci = getinfo(*cn);
        if(!ci) intret(-1);
        else if(ci->state.aitype == AI_NONE) intret(0);
        else intret(1);
    })
    ...
    ICOMMAND(getownernum, "i", (int *cn), {
        ...
        clientinfo *ci = getinfo(*cn);
        if(!ci) intret(-1);
        else intret(ci->ownernum);
    })
    ICOMMAND(getclientpriv, "i", (int *cn), {
        if(!cn) {
            intret(-1);
            return;
        }
        clientinfo *ci = getinfo(*cn);
        if(!ci) intret(-1);
        else intret(ci->privilege);
    })
    ICOMMAND(setclientpriv, "ii", (int *cn, int *priv), {
        if(!cn || !priv) {
            return;
        }
        clientinfo *ci = getinfo(*cn);
    ...
    ICOMMAND(getauthname, "i", (int *cn), {
        if(!cn) return;
        clientinfo *ci = getinfo(*cn);
        if(!ci) return;
        result((const char *)ci->authname);
    })
    ICOMMAND(getauthdesc, "i", (int *cn), {
        if(!cn) return;
        clientinfo *ci = getinfo(*cn);
        if(!ci) return;
        result((const char *)ci->authdesc);
    })
    ICOMMAND(getmoney, "i", (int *cn), {
        if(!cn) return;
        clientinfo *ci = getinfo(*cn);
        if(!ci) return;
        floatret(ci->money);
    })
    ICOMMAND(givemoney, "if", (int *cn, float *cash), {
        if(!cn||!cash) return;
        clientinfo *ci = getinfo(*cn);
        if(!ci) return;
        ci->money += *cash;
    ...
    ICOMMAND(givehp, "ii", (int *cn, int *quantity), {
        if(!cn||!quantity) return;
        clientinfo *ci = getinfo(*cn);
        if(!ci) return;
        ci->state.health += *quantity;
        if(ci->scfClient) {
            loopv(clients) {
                if(!clients[i]->scfClient) continue;
    ...
    ICOMMAND(givehpboost, "ii", (int *cn, int *quantity), {
        if(!cn||!quantity) return;
        clientinfo *ci = getinfo(*cn);
        if(!ci) return;
        ci->state.maxhealth += *quantity;
        if(ci->scfClient) {
            loopv(clients) {
                if(!clients[i]->scfClient) continue;
    ...
    ICOMMAND(givequadsecs, "ii", (int *cn, int *secs), {
        if(!cn||!secs) return;
        clientinfo *ci = getinfo(*cn);
        if(!ci) return;
        ci->state.quadmillis += 1000*(*secs);
        if(ci->scfClient) {
            loopv(clients) {
                if(!clients[i]->scfClient) continue;
    ...
    ICOMMAND(clienthasscf, "i", (int *cn), {
        if(!cn) return;
        clientinfo *ci = getinfo(*cn);
        if(!ci) return;
	...
	*/
	...
	INTCONST(iswindows, 0);
        ...
        ICOMMAND(downloadfile, "ss", (const char *url, const char *destination), {
            ...
            #else
                defformatstring(command)("wget -O %s %s", destination, url);
            #endif
        ...
        ICOMMAND(mkdir, "s", (const char *dir), {
            defformatstring(command)("mkdir -p %s", dir);
    ...
    vector<on_connect> onconnect;
    ICOMMAND(flush_onconnect, "", (), {
        loopvrev(onconnect) {
            onconnect.remove(i);
        }
    })
    ICOMMAND(onconnect, "s", (const char *command), {
        if(!command) return;
        on_connect *cur = &onconnect.add();
        if(!cur) return;
        copystring(cur->command, command);
    })
    ...
    vector<price> prices;
    ICOMMAND(price, "isif", (int *c, const char *n, int *q, float *p), {
        loopv(prices) if(prices[i].code == *c) return;
        price *cur = &prices.add();
        cur->code = *c;
    ...
    ICOMMAND(getitemname, "i", (int *c), {
        loopv(prices) if(prices[i].code == *c) {
            result(prices[i].name);
            return;
        }
    })
    ICOMMAND(getquantity, "i", (int *c), {
        loopv(prices) if(prices[i].code == *c) {
            intret(prices[i].quantity);
            return;
        }
    })
    ICOMMAND(getprice, "i", (int *c), {
        loopv(prices) if(prices[i].code == *c) {
            floatret(prices[i].price);
            return;
        }
    })
	#endif
    ...
    bool scfAnswerModelAuth(int m, clientinfo *ci) {
        ...
        switch(t) {
            case SCF_MIN: return true;
            case SCF_CONNECT: return true;
            case SCF_CLIENT_AUTH: // Some client authenticated - answer authentication.
            {
                ...
                loopv(clients) if(clients[i]->state.aitype == AI_NONE) {
                    ...
                    sendf(ci->clientnum, 1, "riiss", N_SCFCLIENTGEOIP, i, co, city);
                } else {
            ...
            case SCF_ANSWER_AUTH: return true;
            case SCF_SERVER_AUTH: return true;
            case SCF_PM_CN: return true;
            case SCF_PM: return true;
    ...
    void connected(clientinfo *ci)
    {
        ...
        defformatstring(cmd)("scf_connect_cn = %i", ci->clientnum);
        execute(cmd);
    ...
    ICOMMAND(sendcommand, "is", (int *cn, const char *command), {
        if(!cn||!command) return;
        clientinfo *ci = getinfo(*cn);
        if(!ci||!ci->scfClient) return;
        sendf(ci->ownernum, 1, "ris", N_SCFEXEC, command);
    })
    ICOMMAND(sendpubliccommand, "s", (const char *command), {
        if(!command) return;
        loopv(clients) {
            if(!clients[i]->scfClient) continue;
            sendf(clients[i]->ownernum, 1, "ris", N_SCFEXEC, command);
        }
    })
    void listclients(bool bots)
    {
        vector<char> buf;
        string cn;
        int numclients = 0;
        loopv(clients) if(clients[i] && (bots || clients[i]->state.aitype == AI_NONE))
        {
    ...
    ICOMMAND(listclients, "bb", (int *bots), listclients(*bots!=0));

    ICOMMAND(rename, "is", (int *cn, const char *name), {
        if(!cn||!name) return;
        clientinfo *ci = getinfo(*cn);
        if(!ci) return;
        flushserver(true);
    ...
    struct on_switchname {
        string command;
    };
    vector<on_switchname> onswitchname;
    ICOMMAND(flush_onswitchname, "", (), {
        loopvrev(onswitchname) {
            onswitchname.remove(i);
        }
    })
    ICOMMAND(onswitchname, "s", (const char *command), {
        if(!command) return;
        on_switchname *cur = &onswitchname.add();
        if(!cur) return;
        copystring(cur->command, command);
    })
    ...
    void parsepacket(int sender, int chan, packetbuf &p)     // has to parse exactly each byte of the packet
    {
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            case N_POS:
            {
                ...
                if(flags&(1<<4)) // falling
                {
                    /*if(!ci->wasfalling) {
                        ci->wasfalling = true;
                        ci->startfalling = totalmillis;
                    }*/
                    p.get(); if(flags&(1<<5)) p.get();
                    if(flags&(1<<6)) loopk(2) p.get();
                } /*else if(ci->wasfalling) {
                    ci->wasfalling = false;
                    int damage = (int)((totalmillis - ci->startfalling) / 100);
                    if((damage > 9) || !serverfalldamage) { // Don't issue damage if too low.
                        ci->state.health = ci->state.health - damage;
                        sendf(-1, 1, "ri6", N_DAMAGE, ci->clientnum, ci->clientnum, damage, ci->state.armour, ci->state.health);
            ...
            case N_SWITCHNAME:
            {
                ...
                QUEUE_STR(ci->name);
                defformatstring(cmd)("scf_switchname_cn = %i; scf_switchname_name = \"%s\"", ci->clientnum, ci->name);
                execute(cmd);

                loopv(onswitchname) {
                    execute(onswitchname[i].command);
                }
